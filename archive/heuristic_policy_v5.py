...
def plant_expected_profit(crop, day, total_days, market_inv):
    ...
    revenue = harvests * spec["max_yield"] * price(crop, int(market_inv.get(crop, 0)))
...
LAND_UNLOCK_DAY = (7, 11, 15)          # earliest day for the 2nd / 3rd / 4th quadrant
SHED_CAP = 100
...
def _plant_choice(day, total_days, planted, seeds, market_inv):
    """Pick the crop with the best remaining payback for one free tile."""
    # Prioritize Wheat for early game cash flow bursts
...
def _seed_targets(day, total_days, planted, free_n, harvest_crop_n, market_inv):
    """How many seeds of each crop we still want to hold."""
    want = {}
    target_free = free_n + harvest_crop_n
    
    # 1. Strawberries first for cash flow (up to target)
    if plant_expected_profit("STRAWBERRY", day, total_days, market_inv) > 0:
        want["STRAWBERRY"] = max(0, min(TARGET_STRAWBERRY - planted.get("STRAWBERRY", 0), target_free))
    else:
        want["STRAWBERRY"] = 0
        
    # 2. Melons take the rest of the free space for high action-efficiency profit
    if plant_expected_profit("MELON", day, total_days, market_inv) > 0:
        want["MELON"] = max(0, min(TARGET_MELON - planted.get("MELON", 0), target_free - want["STRAWBERRY"]))
    else:
        want["MELON"] = 0
        
    # 3. Fallback to wheat for early cash bursts if profitable
    if plant_expected_profit("WHEAT", day, total_days, market_inv) > 0:
        want["WHEAT"] = max(0, min(TARGET_WHEAT - planted.get("WHEAT", 0), target_free - want.get("STRAWBERRY", 0) - want.get("MELON", 0)))
    else:
        want["WHEAT"] = 0
...
def _make_market_orders(obs, player=0, total_days=30, scan=None):
    ...
    if day <= 1 and hour == 0:
        print(f"Day {day} hour {hour} money {money} shed {shed} seeds {seeds} minv MELON {minv.get('MELON')}")
    ...
    hold_wheat = feed_hold(fed_animals, int(shed.get("WHEAT", 0))) if day < total_days - 1 else 0
    hold = {"WHEAT": hold_wheat}
    ...
    for o in orders:
        inv = int(minv.get(o[1], 0))
        rev, _ = sell_revenue(o[1], inv, o[2])
        money_left += rev

    # ── R7: seeds for the tiles we are about to sow (MOVED UP) ─────────────────────────
    planted = _crop_census(me)
    harvest_n = len(scan.get('harvest_crop', [])) if scan else 0
    want_seeds = _seed_targets(day, total_days, planted, len(free_cells), harvest_n, minv)
    ...
    # ── R5: feed reserve -- an animal unfed 2 days running escapes (MOVED UP) ──────────
    ...
    if fed_animals > 0 and day < total_days - 1:
        ...
        if need > 0 and len(orders) < 10:
            c, _ = buy_cost("WHEAT", int(minv.get("WHEAT", 0)), need)
            if money_left >= c + CASH_FLOOR:
    ...
    if quads - 1 < len(LAND_UNLOCK_DAY) and day >= LAND_UNLOCK_DAY[quads - 1]:
        cost = _land_cost(me)
        if day == 7 and hour == 0:
            print(f"Day 7 hour 0 check BUY_LAND: quads={quads} cost={cost} money_left={money_left} CASH_FLOOR={CASH_FLOOR} len(orders)={len(orders)}")
...
def _build_tasks(scan, me, private, free_cells, day, total_days, minv, hour=0):
    tasks = []
    ...
    if hour <= 20:
        ...
        for cell in free_cells:
            crop = _plant_choice(day, total_days, planted, seeds, minv)
            if crop is None:
                break
...
def _assign_tasks(positions, tasks, invs, board, claims=None, feed_cells=None, custom_tier=None):
    ...
    for ui in free:
        for t in tasks:
            ...
            if key in taken:
                continue
            active_tier = custom_tier if custom_tier is not None else TASK_TIER
            tier = active_tier.get(t[0], 3)
            cell = _task_cell(t)
            if cell is None:
                continue
    ...
    # Ensure correct worker execution order for same-cell harvest and replant/build
    # Worker A (smaller index) must harvest before Worker B (larger index) plants/builds.
    harvest_positions = {}
    tier2_positions = {}
    for ui, t in assignment.items():
        if t[0] == "harvest_crop":
            harvest_positions[_task_cell(t)] = ui
        elif t[0] in ("plant", "build_pasture", "build_coop", "place_animal"):
            tier2_positions[_task_cell(t)] = ui
            
    for cell, ui_tier2 in tier2_positions.items():
        if cell in harvest_positions:
            ui_harvest = harvest_positions[cell]
            if ui_harvest > ui_tier2:
                assignment[ui_harvest], assignment[ui_tier2] = assignment[ui_tier2], assignment[ui_harvest]
...
def _unit_op(pos, task, private, unit_idx, board, me, shed_wheat):
    ...
    target = task[1] if len(task) > 1 else None
    pos = tuple(pos)
    sheds = [tuple(c) for c in _find_structures(me, "SHED")]
    if not sheds:
        sheds = shed_adjacent_cells(board, me)
    shed_positions = sheds
...
def _agent(obs, total_days=30):
    ...
    _sheds = [tuple(c) for c in _find_structures(me, "SHED")]
    if not _sheds:
    ...
    minv = obs.get("market", {}).get("inventory", {})

    # Anticipate harvest cells as free cells for same-turn planting/building!
    harvest_cells = scan.get("harvest_crop", [])
    for cell in harvest_cells:
        if cell not in free_cells:
            free_cells.append(cell)
    # Re-sort free_cells so they are still sorted by proximity to sheds
    free_cells.sort(key=lambda c: min(manhattan(c, s) for s in _sheds))

    tasks = _build_tasks(scan, me, private, free_cells, day, total_days, minv, hour)
    ...
    if dt_assigned_task is not None and 0 in assign_units:
    ...
    if dt_assigned_task is not None:
        dt_task_type = dt_assigned_task[0]
        if dt_task_type in custom_tier:
            custom_tier[dt_task_type] = 0.5
            
    assignment = _assign_tasks(assign_positions, tasks, assign_invs, board, local_prev,
                               set(scan["feed"]), custom_tier=custom_tier)
    ...
    ret = {"farmer": ops_out[0], "hands": ops_out[1:], "market": orders}
    if day == 7 and hour == 0:
        print(f"Day 7 hour 0 final return: {ret}")
    return ret
