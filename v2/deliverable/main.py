"""Kaggriculture agent -- heuristic baseline v1.

Single-file submission: kaggle competitions submit kaggriculture -f main.py -m "v1"

Modules (each unit-tested in test_modules.py):
  1. Price model  -- exact price(inv) from README params, marginal revenue, reserves
  2. Town model   -- shop demand table, deterministic drain per day
  3. Sell scheduler -- reserve-priced batched selling, never dumps
  4. Production planner -- greedy glut-aware tile/action allocation
  5. Opponent reader + labor/movement
  6-7. Per-turn controller and agent(obs) entrypoint
"""


# ============================================================
# 1. PRICE MODEL — exact reconstruction of README "The Price Function"
...
SELLABLE = PRODUCTS  # everything can be SOLD; only WHEAT/FERTILIZER buyable back

def price(resource, inv):
    """Market sell price quoted at inventory level `inv`."""
    ...
    if inv < I0:
        ...
        p = float(base)
    # Banker's rounding matches the published table (carrot P(I0+T): 10.5 -> $10)
    return max(1, int(round(p)))

def sell_revenue(resource, inv, qty):
    """Revenue of selling qty units one at a time (README market mechanics).
    Each unit sells at price(pre-sell inv); unit adds to inventory unless at $1 floor.
    Returns (revenue, new_inv)."""
    rev = 0
...
def marginal_revenue(resource, inv, qty):
    """Price fetched by the qty-th unit, if qty-1 are sold first from `inv`."""
    if qty <= 1:
...
def buy_cost(resource, inv, qty):
    """Cost of buying qty (WHEAT/FERTILIZER). Buy price quoted at post-buy inv.
    Returns (cost, new_inv)."""
    cost = 0
    for _ in range(int(qty)):
        inv -= 1
...
def units_sellable_above(resource, inv, reserve):
    """Max units sellable so every unit fetches >= reserve."""
    if reserve <= 1:
        return 10 ** 6  # at the floor every unit still fetches $1
    n = 0
    while n < 2000:
        p = price(resource, inv)
        if p < reserve:
...
# Reserve price: never sell a unit below this. Cliff-shaped above-curves
# (strawberry/melon/milk/wool) get high reserves; gentle ones can run lower.
_RESERVE_FRAC = {
...
def reserve_price(resource, day, total_days=30):
    """Reserve slides to $1 over the last 3 days: unsold stock is worth $0."""
...
# ============================================================
# 2. TOWN MODEL — shop unlocks + deterministic consumption schedule
# ============================================================
SHOP_DEMANDS = {  # per tick; single-product shops consume 2x (already encoded)
...
SHOP_DEMANDS["ICE_CREAM_SHOP"]["WHEAT"] = 1  # strawberries, milk, wheat
...
def town_center_mult(day):
    # README: 1/tick, 2 after day 10, 4 after day 20 (0-indexed days assumed)
...
def town_drain_per_day(unlocked_shops, day, turns_per_day=24,
    ...
    """{product: units consumed per day} by town center + unlocked shops."""
    drain = {p: 0.0 for p in PRODUCTS}
    center_ticks = turns_per_day / float(center_interval)   # 2/day default
    m = town_center_mult(day)
    for p in CENTER_PRODUCTS:
        drain[p] += center_ticks * m
    shop_ticks = turns_per_day / float(shop_interval)       # 6/day default
...
def expected_future_drain(unlocked_shops, day, horizon_days,
                          ...
                          shop_interval=4, center_interval=12):
    """Expected {product: units/day} averaged over the next horizon_days,
    counting expected random unlocks from the remaining shop pool."""
    ...
    for d in range(day, day + horizon_days):
        ...
        for p in CENTER_PRODUCTS:
            drain[p] += center_ticks * m
        # expected number of extra shops unlocked by day d
...
# ============================================================
# 3. SELL SCHEDULER — reserve-priced, batched, never dumps
# ============================================================
_SELL_ORDER = ["MELON", "WOOL", "MILK", "STRAWBERRY", "TOMATO", "EGG",
               "CARROT", "FERTILIZER", "WHEAT"]  # premium first

def plan_sells(shed, market_inv, day, hour, total_days=30, hold=None,
               max_orders=10, shed_cap=100):
    """Returns list of ["SELL", product, n] orders for this turn.
    Sells each product only down to its reserve price. Relaxes reserves when
    the shed nears capacity (overflow would be discarded = worth $0)."""
    hold = hold or {}
    load = sum(v for k, v in shed.items() if k not in ("GOOSE", "COW", "SHEEP"))
    pressure = 1.0
    if load > 0.9 * shed_cap:
        pressure = 0.5          # shed nearly full: take worse prices over losses
    ...
    for p in _SELL_ORDER:
        ...
        if n <= 0:
            continue
        # batching: for cliff goods hold tiny dribbles unless late or pressured
...
def feed_hold(animal_count, shed_wheat, days_buffer=2):
    """Wheat units to withhold from selling to feed animals."""
    return min(shed_wheat, animal_count * days_buffer)

# ============================================================
# 4. PRODUCTION PLANNER — action/tile-constrained mix, glut-aware
# ============================================================
CROP_SPECS = {  # unfertilized, watered daily
...
def _exp_unit_price(product, market_inv, extra_units, drain_day, days):
    """Mean expected sale price for units trickling in over `days`, given our
    own planned flood `extra_units` and town drain recovery."""
    inv = int(market_inv.get(product, MARKET_PARAMS[product][1]))
    net = int(extra_units - drain_day * days)
    mid = inv + max(0, net) // 2          # midpoint of the glut trajectory
...
def plan_portfolio(free_tiles, n_units, money, market_inv, shops, day,
    ...
    """Greedy allocation of free tiles to plant/build options.
    Returns ordered list of ("PLANT", crop) / ("ANIMAL", species) picks.
    opp_flood: {product: units/day} the opponent is likely to dump (shifts our
    niche away). planned: {product: units} we already have in the pipeline."""
    opp_flood = opp_flood or {}
    planned = dict(planned or {})
    drain = town_drain_per_day(shops, day)
    budget = n_units * 24 * 0.60          # usable actions/day after movement
    picks, spend, counts = [], 0.0, {}
    committed = 0.0                        # actions/day already promised
    for _ in range(int(free_tiles)):
        ...
        for c, s in CROP_SPECS.items():
            ...
            upd = s["peak"] / float(s["occ"])          # units/tile/day
            ...
            apd = 1.0 + 2.0 / s["occ"]                  # water + plant/harvest amortized
        ...
        for a, s in ANIMAL_SPECS.items():
            ...
            apd = 1.0 + upd + 0.2                       # feed + harvest + misc
...
def read_opponent(opp_farm, day):
    """Estimate {product: units/day} the opponent will push to market over the
    next ~week, from their visible tiles (crop mix + maturity + animals)."""
    flood = {}
...
def plan_hires(day, hour, hires_today, money, value_per_action, total_days=30,
               max_hands=6, mult=1):
    """How many hands to hire *this turn*. Hire early in the day (full 24 turns
    of value); stop when fib cost exceeds discounted marginal profit."""
    if day >= total_days - 1 or hour > 4:   # late hires get too few turns
        return 0
    n = 0
    while hires_today + n < max_hands:
        cost = hire_cost(hires_today + n, mult)
        turns_left = 24 - hour
        profit = turns_left * value_per_action * 0.6   # movement discount
...
def step_toward(pos, target):
    """One-move greedy step (Manhattan); returns op string or None if arrived."""
    x, y = pos
    tx, ty = target
    dx, dy = tx - x, ty - y
    if dx == 0 and dy == 0:
...
def assign_tasks(unit_positions, tasks):
    """Greedy nearest-task matching. tasks: [((x,y), op_list, priority), ...].
    Returns {unit_index: ((x,y), op_list)}. Higher priority first, ties by
    distance; each task and unit used once."""
...
# ============================================================
# 6. AGENT GLUE — per-turn controller
# ============================================================
...
def _my_pipeline(me, day, total_days):
    """{product: units} still expected out of my own tiles + shed stock."""
    planned = {}
    ...
    for y in range(len(tiles)):
        for x in range(len(tiles[y])):
            if tiles[y][x] is None:
                cells.append((x, y))
    h = board_size // 2
    cells.sort(key=lambda c: manhattan(c, (h, h)))  # cluster near shed
    return cells

def _scan(me, day):
    """Categorised cells of my farm."""
...
def build_tasks(scan, me, private, picks, free_cells, day, hour, board_size, total_days):
    """List of (cell, op, priority). op is a list like ["WATER"]."""
    tasks = []
    ...
    # placement of bought animals onto empty structures
    shed = private.get("shed") or {}
    for a in ANIMAL_ITEMS:
        pass  # handled dynamically in _unit_op via PICKUP/PLACE below
    ...
    for a in ANIMAL_ITEMS:
        ...
        for cell, kind in placeable:
            ...
            n -= 1
    # planting: one task per seed per free cell
...
def _unit_op(pos, task, private, unit_idx, board_size, feed_wheat_needed):
    """Resolve a unit's concrete op for this turn given its assigned task."""
    invs = private.get("inventories") or []
    inv = invs[unit_idx] if unit_idx < len(invs) else {}
    cell, ops = task
    op = ops[0]
    # FEED requires wheat on the unit: detour via shed if empty-handed
    if op == "FEED" and int((inv or {}).get("WHEAT", 0)) <= 0:
        ...
        if tuple(pos) in [tuple(c) for c in sheds]:
            ...
            return ["PASS"]  # wheat is being bought this turn
...
def _drop_task_needed(inv):
    return sum(int(v) for k, v in (inv or {}).items()) >= 5
...
def _agent(obs):
    ...
    money_left = money
    # feed security: keep 2 days of wheat for animals
    ...
    # units that should go dump inventory instead of working
