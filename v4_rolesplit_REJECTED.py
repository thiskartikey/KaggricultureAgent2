...
# ── try to load trained RL weights ──────────────────────────────────────────
_RL_POLICY = None
try:
...
def shed_adjacent_cells(board_size=10):
    h=board_size//2; return [(h-1,h-1),(h,h-1),(h-1,h),(h,h)]

_FARMER_OPS=("BUILD_COOP","BUILD_PASTURE","DIG","__PLACE__")

def assign_tasks(unit_positions, tasks, farmer_is_unit0=False):
    """Greedy nearest-unit per task (priority order). If farmer_is_unit0, the
    owner gets a distance bonus on build/dig/place tasks so hands keep to
    field work (watering/harvest) in their zones."""
    ...
    for _,(cell,ops,_pr) in tasks:
        ...
        for u in free:
            ...
            if farmer_is_unit0 and u==0 and ops[0] in _FARMER_OPS:
                d-=3   # owner prefers build/dig work
            if d<best_d: best_u,best_d=u,d
        free.discard(best_u); out[best_u]=(cell,ops)
    return out

def plan_dynamic_hires(tasks, unit_positions, day, hour, money, hires_today,
                       total_days=30, occupied=0):
    """Baseline 2 hands at dawn; extra hires are distance-triggered — when
    several tasks sit far from every unit, a fresh hand (spawns at the shed)
    covers the near work while travellers finish the far work. Crew size is
    capped by how much of the farm is actually productive."""
    if day>=total_days-1: return 0
    day_cap=min(8, 2+occupied//7)   # crew scales with working tiles
    if hires_today>=day_cap: return 0
    if hour==0:
        return max(0, min(2,day_cap)-hires_today)
    if hour>18: return 0   # too few turns left today to pay off
    far=0
    for cell,_ops,_pr in tasks:
        dmin=min((manhattan(p,cell) for p in unit_positions), default=0)
        if dmin>=5: far+=1
    extra=1 if far>=4 else 0
    cost=_FIB[min(hires_today,len(_FIB)-1)]
    if extra and money<cost+150: extra=0
    return extra
...
def build_tasks(scan, me, private, picks, free_cells, day, hour, board_size, total_days):
    ...
    for cell,pr in scan["feed"]: tasks.append((cell,["FEED"],pr))
    ...
    for a in ANIMAL_ITEMS:
        want=ANIMAL_SPECS[a]["structure"]; n=int(shed.get(a,0))
        for cell,kind in placeable:
            if n<=0 or kind!=want: continue
            tasks.append((cell,["__PLACE__",a],85)); n-=1
    ...
    for cell in scan["care"]: tasks.append((cell,["CARE"],35))
    return tasks
...
def _unit_op(pos, task, private, unit_idx, board_size, feed_wheat_needed):
    ...
    if op=="FEED" and int((inv or {}).get("WHEAT",0))<=0:
        ...
        if tuple(pos) in [tuple(c) for c in sheds]:
            ...
            return ["PICKUP","WHEAT",n] if n>0 else ["PASS"]
...
def _build_market_orders(obs, picks, scan, total_days=30, tasks=None):
    ...
    orders=[]; hold={"WHEAT":feed_hold(animals,int(shed.get("WHEAT",0)))}
    ...
    if animals and int(shed.get("WHEAT",0))<animals and day<total_days-1:
        ...
        if money_l>c+100: orders.append(["BUY_PRODUCT","WHEAT",need]); money_l-=c

    want_seeds={}
    for kind,name in picks:
        if kind=="PLANT": want_seeds[name]=want_seeds.get(name,0)+1
    for c,n in want_seeds.items():
        n-=int(seeds.get(c,0)); n=min(n,8)
        if n>0 and money_l>CROP_SPECS[c]["seed"]*n+100:
            orders.append(["BUY_SEED",c,n]); money_l-=CROP_SPECS[c]["seed"]*n
    ...
    for cell,kind in scan["structures_empty"][:4]:
        ...
        if pending==0 and money_l>ANIMAL_SPECS[a]["cost"]+200 \
           and day<total_days-ANIMAL_SPECS[a]["first"]-4:
            orders.append(["BUY_ANIMAL",a,1]); money_l-=ANIMAL_SPECS[a]["cost"]

    want=_desired_hands(scan,_free_cells(me,len(me.get("tiles") or [])),picks,day,total_days)
    hires=int(me.get("hires_today",0))
    if hour<=2:
        h=0
        while hires+h<want:
            c=_FIB[min(hires+h,len(_FIB)-1)]
            if money_l<c+50: break
            orders.append(["HIRE"]); money_l-=c; h+=1

    lo=_land_order(me,len(_free_cells(me,len(me.get("tiles") or []))),day,money_l,total_days)
    if lo: orders.append(lo)
    return orders[:10]
...
def _agent(obs):
    ...
    animals=_count_animals(me); n_units=1+len(me.get("hands") or [])
    ...
    orders=_build_market_orders(obs,picks,scan,total_days,tasks)
    ...
    if _RL_POLICY is not None and _RL_POLICY.loaded:
        ...
        asgn=assign_tasks([positions[i] for i in free_units],tasks,
                          farmer_is_unit0=(free_units and free_units[0]==0))
        feed_need=max(1,min(6,len(scan["feed"])))
        for li,task in asgn.items():
            gi=free_units[li]; ops_out[gi]=_unit_op(positions[gi],task,private,gi,board,feed_need)
    ...
    # hands still use v2 task system (tasks already built above)
    ...
    for li,task in asgn.items():
        if hands_ops[li] is None:
            hands_ops[li]=_unit_op(hands_pos[li],task,private,li+1,board,feed_need)
