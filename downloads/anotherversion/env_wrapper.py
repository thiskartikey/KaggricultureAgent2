...
class KagrEnv(BaseEnv):
    ...
    def _make_market_orders(self, obs, player=0):
        """Re-use v2's sell logic (imported lazily to avoid circular deps)."""
        from heuristic import plan_sells, feed_hold, _count_animals, buy_cost
        me      = obs["farms"][player]
        priv    = obs.get("private",{}) or {}
        ...
        orders  = plan_sells(shed, minv, day, hour, 30, hold)
        money_l = money

        # feed security
        if animals and int(shed.get("WHEAT",0))<animals and day<29:
            need = animals*2 - int(shed.get("WHEAT",0))
            c,_  = buy_cost("WHEAT", minv.get("WHEAT",10000), need)
            if money_l > c+100:
                orders.append(["BUY_PRODUCT","WHEAT",need]); money_l-=c

        # hire up to 3 hands early
        hires = int(me.get("hires_today",0))
        if hour<=1 and day<29:
            fib=[1,1,2,3,5,8,13]
            for _ in range(3-hires):
                c=fib[min(hires,6)]
                if money_l>c+200:
                    orders.append(["HIRE"]); money_l-=c; hires+=1

        # land
        q = len(me.get("unlocked_quadrants",["NW"]))
        if q<4 and day<22:
            cost=1000*(2**(q-1))
            if money_l>cost+500: orders.append(["BUY_LAND"])
