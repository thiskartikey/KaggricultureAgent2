...
class TestPolicyUnit(unittest.TestCase):
    ...
    def test_sell_and_buy_cost(self):
        ...
        rev, inv2 = policy.sell_revenue("WHEAT", 10000, 5)
        self.assertGreater(rev, 0)
        self.assertEqual(inv2, 10005)

        cost, inv3 = policy.buy_cost("WHEAT", 10000, 5)
        self.assertGreater(cost, 0)
        self.assertEqual(inv3, 9995)

    def test_hire_cost_fibonacci(self):
        """Test worker hiring cost progression."""
        self.assertEqual(policy.hire_cost(0), 1)
        self.assertEqual(policy.hire_cost(1), 1)
        self.assertEqual(policy.hire_cost(2), 2)
        self.assertEqual(policy.hire_cost(3), 3)
        self.assertEqual(policy.hire_cost(4), 5)
        self.assertEqual(policy.hire_cost(5), 8)

    def test_movement_helpers(self):
        """Test Manhattan distance and step direction."""
        self.assertEqual(policy.manhattan((0, 0), (3, 4)), 7)
        self.assertEqual(policy.step_toward((0, 0), (3, 0)), "EAST")
        self.assertEqual(policy.step_toward((3, 0), (0, 0)), "WEST")
        self.assertEqual(policy.step_toward((0, 0), (0, 4)), "SOUTH")
        self.assertEqual(policy.step_toward((0, 4), (0, 0)), "NORTH")
    ...
    def test_day0_hour1_opening_playbook(self):
        ...
        orders = policy._make_market_orders(obs, player=0)
        expected_orders = [
            ["HIRE"],
            ["HIRE"],
            ["HIRE"],
    ...
    def test_agent_fallback_on_exception(self):
        ...
        res = policy.agent({"corrupted": True})
