...
TARGET_SHEEP = 0
TARGET_GOOSE = 0
# No pastures needed if no animals
...
def _seed_targets(day, total_days, planted, free_n, harvest_crop_n, market_inv):
    ...
    if plant_expected_profit("WHEAT", day, total_days, market_inv) > 0:
        rest = target_free - want.get("STRAWBERRY", 0) - want.get("MELON", 0)
        # Hold a small buffer of seeds for future turns, but don't hold them near game end
        want["WHEAT"] = min(30, max(0, rest) + 5)
    else:
        want["WHEAT"] = 0
